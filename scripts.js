...
function addBodyClass() {
  ...
  if (document.body.classList.contains("asana-enhancer")) {
    return false;
  }

  document.body.classList.add("asana-enhancer");
}

function insertToggle() {
  /** Append a <button> to the body */
  const button = document.createElement("button");
  button.classList.add("toggle-plugin");
  button.innerHTML = icon;
  button.title = "Toggle Asana Enhancer";
  button.onclick = () => {
    if (document.body.classList.contains("asana-enhancer")) {
      document.body.classList.remove("asana-enhancer");
    } else {
      document.body.classList.add("asana-enhancer");
    }
  };
